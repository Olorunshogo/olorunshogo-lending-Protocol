...
abstract contract StdChains {
  ...
  function setChain(string memory chainAlias, Chain memory chain) internal virtual {
    setChain(
      chainAlias,
      ChainData({name: chain.name, chainId: chain.chainId, rpcUrl: chain.rpcUrl})
    );
  }
  ...
  function initializeStdChains() private {
    ...
    setChainWithDefaultRpcUrl(
      'holesky',
      ChainData('Holesky', 17000, 'https://rpc.holesky.ethpandaops.io')
    );
    setChainWithDefaultRpcUrl(
      'hoodi',
      ChainData('Hoodi', 560048, 'https://rpc.hoodi.ethpandaops.io')
    );
    ...
    setChainWithDefaultRpcUrl(
      'avalanche',
      ChainData('Avalanche', 43114, 'https://api.avax.network/ext/bc/C/rpc')
    );
    setChainWithDefaultRpcUrl(
      'avalanche_fuji',
    ...
    setChainWithDefaultRpcUrl(
      'moonbeam',
      ChainData('Moonbeam', 1284, 'https://rpc.api.moonbeam.network')
    );
    setChainWithDefaultRpcUrl(
      'moonriver',
    ...
    setChainWithDefaultRpcUrl(
      'moonbase',
      ChainData('Moonbase', 1287, 'https://rpc.testnet.moonbeam.network')
    );
    setChainWithDefaultRpcUrl(
      'base_sepolia',
      ChainData('Base Sepolia', 84532, 'https://sepolia.base.org')
    );
    setChainWithDefaultRpcUrl('base', ChainData('Base', 8453, 'https://mainnet.base.org'));
    setChainWithDefaultRpcUrl(
      'blast_sepolia',
      ChainData('Blast Sepolia', 168587773, 'https://sepolia.blast.io')
    );
    ...
    setChainWithDefaultRpcUrl(
      'flare',
      ChainData('Flare', 14, 'https://flare-api.flare.network/ext/C/rpc')
    );
    setChainWithDefaultRpcUrl(
      'flare_coston2',
    ...
    setChainWithDefaultRpcUrl(
      'ink_sepolia',
      ChainData('Ink Sepolia', 763373, 'https://rpc-gel-sepolia.inkonchain.com')
    );

    setChainWithDefaultRpcUrl('mode', ChainData('Mode', 34443, 'https://mode.drpc.org'));
    setChainWithDefaultRpcUrl(
      'mode_sepolia',
      ChainData('Mode Sepolia', 919, 'https://sepolia.mode.network')
    );

    setChainWithDefaultRpcUrl('zora', ChainData('Zora', 7777777, 'https://zora.drpc.org'));
    setChainWithDefaultRpcUrl(
      'zora_sepolia',
      ChainData('Zora Sepolia', 999999999, 'https://sepolia.rpc.zora.energy')
    );

    setChainWithDefaultRpcUrl('race', ChainData('Race', 6805, 'https://racemainnet.io'));
    setChainWithDefaultRpcUrl(
      'race_sepolia',
      ChainData('Race Sepolia', 6806, 'https://racemainnet.io')
    );

    setChainWithDefaultRpcUrl('metal', ChainData('Metal', 1750, 'https://metall2.drpc.org'));
    setChainWithDefaultRpcUrl(
      'metal_sepolia',
      ChainData('Metal Sepolia', 1740, 'https://testnet.rpc.metall2.com')
    );

    setChainWithDefaultRpcUrl(
      'binary',
      ChainData('Binary', 624, 'https://rpc.zero.thebinaryholdings.com')
    );
    setChainWithDefaultRpcUrl(
      'binary_sepolia',
      ChainData('Binary Sepolia', 625, 'https://rpc.zero.thebinaryholdings.com')
    );

    setChainWithDefaultRpcUrl('orderly', ChainData('Orderly', 291, 'https://rpc.orderly.network'));
    setChainWithDefaultRpcUrl(
      'orderly_sepolia',
      ChainData('Orderly Sepolia', 4460, 'https://testnet-rpc.orderly.org')
    );

    setChainWithDefaultRpcUrl(
      'unichain',
      ChainData('Unichain', 130, 'https://mainnet.unichain.org')
    );
    setChainWithDefaultRpcUrl(
      'unichain_sepolia',
      ChainData('Unichain Sepolia', 1301, 'https://sepolia.unichain.org')
    );

    setChainWithDefaultRpcUrl('tempo', ChainData('Tempo', 4217, 'https://rpc.mainnet.tempo.xyz'));
    setChainWithDefaultRpcUrl(
      'tempo_moderato',
      ChainData('Tempo Moderato', 42431, 'https://rpc.moderato.tempo.xyz')
    );
    setChainWithDefaultRpcUrl(
      'tempo_andantino',
      ChainData('Tempo Andantino', 42429, 'https://rpc.testnet.tempo.xyz')
    );
  }
