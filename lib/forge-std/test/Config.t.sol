...
contract ConfigTest is Test, Config {
  ...
  function test_writeConfig() public {
...

    {
      ...
      content = vm.readFile(testConfig);
      uint256[] memory updated_numbers_disk = vm.parseTomlUintArray(
        content,
        '$.optimism.uint.number_array'
      );
      ...
      content = vm.readFile(testConfig);
      string[] memory updated_strings_disk = vm.parseTomlStringArray(
        content,
        '$.mainnet.string.str_array'
      );
      ...
      content = vm.readFile(testConfig);
      bytes32[] memory updated_words_disk = vm.parseTomlBytes32Array(
        content,
        '$.optimism.bytes32.new_words'
      );
  ...
  function test_writeUpdatesBackToFile() public {
    ...
    string memory content = vm.readFile(testConfig);
    assertTrue(
      vm.parseTomlBool(content, '$.mainnet.bool.is_live'),
    ...
    assertFalse(
      vm.parseTomlBool(content, '$.mainnet.bool.is_live'),
      'File should not be updated again'
    );
  ...
  function testRevert_ChainNotInitialized() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(StdConfig.ChainNotInitialized.selector, uint256(999999))
    );
