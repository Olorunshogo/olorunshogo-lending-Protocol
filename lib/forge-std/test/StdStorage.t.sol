...
contract StdStorageTest is Test {
  ...
  function testFuzz_StorageNativePack(
    uint248 val1,
    uint248 val2,
    bool boolVal1,
    bool boolVal2
    ...
    stdstore.enable_packed_slots().target(address(test)).sig(test.tB.selector).checked_write(
      boolVal1
    );
    stdstore.enable_packed_slots().target(address(test)).sig(test.tC.selector).checked_write(
      boolVal2
    );
  ...
  function testEdgeCaseArray() public {
    stdstore.target(address(test)).sig('edgeCaseArray(uint256)').with_key(uint256(0)).checked_write(
      1
    );
...
contract StorageTest {
  ...
  function getRandomPacked(
    uint8 shifts,
    uint8[] memory shiftSizes,
    uint8 elem
