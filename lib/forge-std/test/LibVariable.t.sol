...
contract LibVariableTest is Test {
  ...
  function test_Downcasting() public view {
    // Uint downcasting
    Variable memory v_uint_small = Variable(
      Type(TypeKind.Uint256, false),
    ...
    small_uints[0] = 10;
    small_uints[1] = 20;
    Variable memory v_uint_array_small = Variable(
      Type(TypeKind.Uint256, true),
      abi.encode(small_uints)
    );
    ...
    // Int downcasting
    Variable memory v_int_small_pos = Variable(
      Type(TypeKind.Int256, false),
      abi.encode(int256(100))
    );
    Variable memory v_int_small_neg = Variable(
      Type(TypeKind.Int256, false),
      abi.encode(int256(-100))
    );
  ...
  function testRevert_TypeMismatch() public {
    ...
    // Arrays
    vm.expectRevert(
      abi.encodeWithSelector(LibVariable.TypeMismatch.selector, 'uint256[]', 'bool[]')
    );
    helper.toUint256Array(boolArrayVar);

    vm.expectRevert(
      abi.encodeWithSelector(LibVariable.TypeMismatch.selector, 'address[]', 'string[]')
    );
  ...
  function testRevert_UnsafeCast() public {
    // uint overflow
    Variable memory uintLarge = Variable(
      Type(TypeKind.Uint256, false),
      abi.encode(uint256(type(uint128).max) + 1)
    );
    expectedErr = abi.encodeWithSelector(
      LibVariable.UnsafeCast.selector,
      "value does not fit in 'uint128'"
    );
    vm.expectRevert(expectedErr);
    helper.toUint128(uintLarge);

    // int overflow
    Variable memory intLarge = Variable(
      Type(TypeKind.Int256, false),
      abi.encode(int256(type(int128).max) + 1)
    );
    expectedErr = abi.encodeWithSelector(
      LibVariable.UnsafeCast.selector,
    ...
    // int underflow
    Variable memory intSmall = Variable(
      Type(TypeKind.Int256, false),
      abi.encode(int256(type(int128).min) - 1)
    );
    expectedErr = abi.encodeWithSelector(
      LibVariable.UnsafeCast.selector,
      "value does not fit in 'int128'"
    ...
    expectedErr = abi.encodeWithSelector(
      LibVariable.UnsafeCast.selector,
      "value in array does not fit in 'int64'"
    );
